...
class ScheduleModelMembers extends ListModel
{
	...
	protected function configureTables()
	{
		...
		$queryHelper->addTable('member', '#__schedule_members')
			->addTable('category',  '#__categories', 'member.catid      = category.id')
			->addTable('user',      '#__users',      'member.created_by = user.id')
			->addTable('viewlevel', '#__viewlevels', 'member.access     = viewlevel.id')
			->addTable('lang',      '#__languages',  'member.language   = lang.lang_code');
